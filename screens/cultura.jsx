...
export default function Cultura({ navigation }) {
  ...
  const getAll = async () => {
    ...
    const allPost = await fetch(
      `https://posts2-api.global.ssl.fastly.net/1/dossier/1/posts?apikey=${API_KEY}&apitoken=${API_TOKEN}&per_page=12&page=${page}&include=bodies,tags,photos,albums,authors,labels,audios,documents,dossiers,collections&filter[isoLanguage]=pt`
    );
